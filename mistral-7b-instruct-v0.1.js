    ...
    async fetch(request, env, ctx) {
        ...
        if (request.method === 'POST' && pathname === '/api/chat') {
            try {
                ...
                console.log(`请求消息内容预览:`, JSON.stringify(messages.slice(-2)).substring(0, 200));

                try {
                    console.log('开始调用 Mistral 模型...');
                    console.log('使用模型ID: @cf/mistral/mistral-7b-instruct-v0.1');
                    ...
                    console.log('Mistral 模型调用成功!');
                    console.log('响应类型:', typeof response);
                    console.log('响应结构:', Object.keys(response).join(', '));
                    console.log('响应预览:', JSON.stringify(response).substring(0, 200));
                    
                    // 检查响应格式并确保它包含内容
                    let formattedResponse;
                    
                    if (typeof response === 'string') {
                        console.log('响应为字符串类型');
                        formattedResponse = { content: response };
                    } else if (response && response.response) {
                        console.log('响应包含response字段');
                        formattedResponse = { content: response.response };
                    } else if (response && response.content) {
                        console.log('响应包含content字段');
                        formattedResponse = { content: response.content };
                    } else if (response && typeof response === 'object') {
                        console.log('响应为对象类型但没有标准字段');
                        // 如果是对象但没有预期的字段，尝试将其转换为可用内容
                        if (response.usage && !response.content && !response.response) {
                            console.log('响应只包含usage字段，这是不完整的响应');
                            // 只有usage字段，这是不完整的响应
                            formattedResponse = { 
                                content: "AI助手正在思考中，请稍后再试。",
                                error: "获取到不完整的API响应"
                            };
                        } else {
                            // 保持原样，但添加默认content以防
                            console.log('尝试从对象构建有效响应');
                            const possibleContent = response.content || response.response || JSON.stringify(response);
                            formattedResponse = {
                                ...response,
                    ...
                    console.log('最终格式化的响应:', JSON.stringify(formattedResponse).substring(0, 200));
                    return Response.json({ response: formattedResponse });
                } catch (error) {
                    console.error('Mistral 模型调用失败:');
...
<head>
    ...
    <title>mistral-7b-instruct-v0.1 聊天助手</title>
...
<body>
    <div class="header">
        <div class="header-title">
            <h1>mistral-7b-instruct-v0.1 聊天助手 <span class="model-badge">7B 参数</span></h1>
        </div>
...
<div class="main-content">
        ...
        <div class="chat-container">
            <div class="container">
                <div class="chat-box" id="chatBox">
                    <div class="welcome" id="welcome">
                        <h2>欢迎使用mistral-7b-instruct-v0.1聊天助手</h2>
                        <p>有什么代码问题可以帮助您的？</p>
                        <div class="examples">
                            <div class="example-item" data-prompt="写一个Python函数，计算斐波那契数列">写一个Python函数，计算斐波那契数列</div>
                            <div class="example-item" data-prompt="解释JavaScript中的Promise是什么">解释JavaScript中的Promise是什么</div>
                            <div class="example-item" data-prompt="如何使用React Hooks实现状态管理">如何使用React Hooks实现状态管理</div>
                            <div class="example-item" data-prompt="生成一个简单的HTML5网页模板">生成一个简单的HTML5网页模板</div>
                        </div>
                    </div>
                </div>
            ...
            <div class="footer">
                Powered by Cloudflare Workers AI + DeepSeek Coder 6.7B
            </div>
        </div>
    </div>
