export default {
      ...
      if (request.method === 'POST' && pathname === '/api/chat') {
        try {
          ...
          try {
            ...
            console.log('Qwen模型响应成功:', typeof response);
            return Response.json({ response });
          } catch (error) {
            console.error('Qwen模型调用失败:', error);
  
            // 备用：使用Llama模型
            console.log('使用备用Llama模型');
            const backupResponse = await env.AI.run('@cf/meta/llama-3-8b-instruct', {
              messages: messages
            });
  ...
  <body>
    <div class="header">
      <div class="header-title">
        <h1>Qwen 1.5-14B 聊天助手 <span class="model-badge">14B 参数</span></h1>
      </div>
    ...
    <div class="main-content">
      ...
      <div class="chat-container">
        ...
        <div class="footer">
          Powered by Cloudflare Workers AI + Qwen 1.5-14B-Chat
        </div>
      </div>
    </div>
  ...
  if (currentWindow) {
      ...
      function loadChatHistory() {
        ...
        try {
          ...
          if (saved) {
            ...
            // 渲染已保存的消息
            chatBox.innerHTML = '';
            windowData.history.forEach((msg, index) => {
              if (msg.role !== 'system') {
      ...
      function saveChatHistory() {
        ...
        try {
          const currentWindow = chatWindows.find(w => w.id === currentWindowId);
          localStorage.setItem("chat_window_" + currentWindowId, JSON.stringify({ history: currentWindow.history }));
        } catch (e) {
      ...
      function sendMessage() {
        ...
        // 自动重命名窗口 - 使用第一个问题作为标题
        // 检查当前窗口是否只有系统消息和一条用户消息(即刚刚添加的这条是第一条用户消息)
      ...
      function handleChatMessage() {
        ...
        fetch('/api/chat', {
          ...
          body: JSON.stringify({ messages: chatWindows.find(w => w.id === currentWindowId).history })
        })
      ...
      // 添加消息到UI
      function addMessage(role, content, usedBackupModel, messageIndex) {
        ...
        if (role === 'user') {
          ...
          editBtn.addEventListener('click', function() {
            editMessage(messageDiv, content, messageIndex);
          });
      ...
      function editMessage(messageDiv, content, messageIndex) {
        ...
        saveBtn.addEventListener('click', function() {
          ...
          if (newMessage) {
            ...
            if (historyIndex !== undefined) {
              // 删除此消息之后的所有消息
              currentWindow.history = currentWindow.history.slice(0, historyIndex);
              
              // 添加编辑后的消息
              currentWindow.history.push({ role: 'user', content: newMessage });
